...
//-- feel free to change these global variables' values to false --//
	...
	var please_StyleInputTextsAndPasswords = false;
	var please_StyleTextareas = false;
	...
	var please_StyleButtonButtons = false;
	var please_StyleInputButtons = false;
...
function TofslaFormsStart()
{
	...
	if( !jQuery.browser.opera && IEversion() != 6 && IEversion() != 7)
	{
...
function StyleControls(controlToFind)
{
	...
	jQuery(controlToFind).each(function(index, control ){
		...
		for(var i = 0; i < iterateTill; i++)
		{
			...
			if( IsStylingAllowed(jQuery(control)) )
			{

				g_currentControl = jQuery(control);
				...
				if( formControls[controlName][i].nodeRelativeTo == 'control' )
					nodeToManipulate = jQuery(control);
				else
				{
					nodeToManipulate = jQuery(g_previouslyAddedNode);
				}
				...
				if( i == 0 )
				{
					...
					{
						jQuery(g_previouslyAddedNode).addClass("tofslaControl");
					}
					else
						jQuery(control).addClass("tofslaControl");
				}
		...
		jQuery(control).addClass("tofslaApplied");
...
function polishSelect()
{
	...
	jQuery(g_currentControl[0].parentNode).css('zIndex', 100-g_SelectIndex);
	g_SelectIndex++;
	
	
	jQuery('option', g_currentControl).each(function(index, node){
		optionsHTML	= optionsHTML + '<li><a href="#" index="'+ index +'">' + jQuery(node).html() + '</a></li>';
	});

	var ul = jQuery('ul', g_currentControl.parent() )[0];
	jQuery(ul).prepend ( jQuery(optionsHTML) );
...
function polishButtonButton()
{
	if(g_currentControl.text() != "")
	{
		...
		jQuery(g_currentControl[0].childNodes[1]).replaceWith('');
...
function setSelectsCalculatedWidth()
{
	...
	var SelectAwidth = parseInt( jQuery(mainWrapper.childNodes[0].childNodes[1]).css('width') ); // <a>
	var SelectSpanWidth = RemCurrentSelectWidth.width - SelectAwidth - getHorizontalBorderPadding(selectSpan);


	jQuery(mainWrapper).css('width', RemCurrentSelectWidth.width + 'px');
	jQuery(selectSpan).css('width', SelectSpanWidth + 'px');
	

	var ul = jQuery('ul', mainWrapper);
...
function UpdateDisabledState()
{
	...
	jQuery('.tofslaControl').each(function(index, node ){
		
		if(node.tagName.toLowerCase() != "button")
		{
			if(jQuery('.tofslaApplied', node)[0].disabled)
				jQuery(node).addClass('tfDisabled');
			else
				jQuery(node.parentNode).removeClass('tfDisabled');
		}
		else
		{
			if(node.disabled)
				...
				jQuery(node).removeClass('tfDisabled');
		}
...
{
	jQuery('.tfCheckboxWrapper').each(function(index, node){

		var a = node.childNodes[0];
		var input = node.childNodes[1];

		jQuery(a).removeClass('tfChecked');

		if(input.checked)
			jQuery(a).addClass('tfChecked');
...
{
	jQuery('.tfRadioWrapper').each(function(index, node){

		var a = node.childNodes[0];
		var input = node.childNodes[1];
		
		jQuery(a).removeClass('tfChecked');
		jQuery(input).removeClass('tfChecked');
...
function resetSelects()
{
	...
	jQuery('.tfSelectWrapper').each(function(index, node){
		...
		if( !seLect.getAttribute('multiple') )
		{
			...
			selectLink = jQuery('a[index="' + selIndex + '"]', ul)[0];
...
function handleSelectsClick(wrapper, thisLink, seLect, ul)
{
	...
	jQuery('span', wrapper)[0].innerHTML = thisLink.innerHTML;
	
	jQuery('a.selected', wrapper).each( function(i, linky){
		jQuery(linky).removeClass('selected');		
	});

	jQuery(thisLink).addClass('selected');
	...
	var changeVisualCheck = function(check){
		...
		if(!check.disabled)
		{
			if (check.checked){
				...
				jQuery(check.previousSibling).addClass('tfChecked');
			}
			else {
				jQuery(check.previousSibling).removeClass('tfChecked');
			}
...
function listenRadio()
{
	...
	var changeVisualRadio = function(rd){
		...
		if(!rd.disabled)
		{
			...
			jQuery('a[rel="' + rd.getAttribute('name') +'"]').each(function(index, node){
				
					jQuery(node).removeClass('tfChecked');
				
			});
			
			jQuery(rd.previousSibling).addClass('tfChecked');
...
{
	jQuery('button:reset').each(function(index, node){

		if( jQuery(node).hasClass('tofslaApplied') && jQuery(node).hasClass('addResetListener') )
		{
			jQuery(node).removeClass('addResetListener');
			
			jQuery(node).bind("click", function(event) {
...
function listenSelect()
{
	...
	jQuery('a', ul).each(function(i, linky){

		jQuery(linky).bind('mousedown', function(event) {
		...
		if(IEversion() > -1) // IE
		{
			jQuery(linky).bind('mouseover', function(event) {
				jQuery(linky).addClass('hover');
			});
			
			jQuery(linky).bind('mouseout', function(event) {
				jQuery(linky).removeClass('hover');
			});
		}
		
		jQuery(linky).click(function() {
			return false;
		});
	...
	/* Set the defalut */
	var Link = jQuery( 'a[index="' + seLect.selectedIndex + '"]', ul)[0];
	
	handleSelectsClick (wrapper, Link, seLect, ul);
	
	/* Apply the click handler to the Open */
	jQuery( jQuery('a.tfSelectOpen', wrapper)[0] ).bind('mousedown', function(event) {
		if(!seLect.disabled)
		{
	...
	jQuery(jQuery('div span', wrapper)[0]).bind('mousedown', function(event) {
	...
	var hideAllOpenSelects = function(nodeToLeaveOpen){
		...
		jQuery('.tfSelectWrapper').each(function(index, node){
			...
			var ulInQuestion = jQuery('ul', node)[0];
			
			if ( ( jQuery(ulInQuestion).css('display') == 'block') && (node != nodeToLeaveOpen) )
			{
				jQuery(ulInQuestion).hide();
			}
	...
	var checkExternalClick = function(target) {
		...
		var parents = jQuery(target).parents();
		var isChildOfStyledSelect = false;

		parents.each(function(index, node){
			if( jQuery(node).hasClass('tfSelectWrapper') )
...
{
	var nodeToDecorate = jQuery(g_currentControl[0]);

	nodeToDecorate.bind("focus", function(event) {
		jQuery(nodeToDecorate.parents('.tofslaControl')[0]).addClass('tfFocus');
	});

	nodeToDecorate.bind("blur", function(event) {
		jQuery(nodeToDecorate.parents('.tofslaControl')[0]).removeClass('tfFocus');
	});
}
...
function decorateButtons()
{
	...
	jQuery('button').each(function(index, node){
	
		if( jQuery(node).hasClass('tofslaApplied') && jQuery(node).hasClass('addDecorateButton') )
		{
			jQuery(node).bind("mouseenter", function(event) {
				jQuery(this).addClass('tfHover');		
			});
		
			jQuery(node).bind("mouseleave", function(event) {
				jQuery(this).removeClass('tfHover');		
			});
		
			jQuery(node).bind("mousedown", function(event) {
				jQuery(this).addClass('tfActive');
			});
		
			jQuery(node).bind("mouseup", function(event) {
				jQuery(this).removeClass('tfActive');		
			});
			
			jQuery(node).removeClass('addDecorateButton');
		}
